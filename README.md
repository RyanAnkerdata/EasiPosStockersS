...
* Check the `ConnectionStrings` in `appsettings.json` files under the `EasiPosStockers.HttpApi.Host` and `EasiPosStockers.DbMigrator` projects and change it if you need.
...
* Run `EasiPosStockers.DbMigrator` to create the initial database. This should be done in the first run. It is also needed if a new database migration is added to the solution later.
...
* `EasiPosStockers.DbMigrator`: A console application which applies the migrations and also seeds the initial data. It is useful on development as well as on production environment.
* `EasiPosStockers.HttpApi.Host`: ASP.NET Core API application that is used to expose the APIs to the clients.
* `EasiPosStockers.Blazor`: ASP.NET Core Blazor Server application that is the essential web application of the solution.
